...
Q = T^3 + Y
\end{equation}
where $T^3$ is the third component of weak isospin.
...
\bottomrule
\end{tabular}
\caption{The three generations of fundamental fermions with their electromagnetic charges and approximate masses. Quark masses are $\overline{\mathrm{MS}}$ running masses. Neutrino masses are non-zero but $< 0.45\,\mathrm{eV}$.}
\label{tab:fermions}
\end{table}
...
\begin{equation}
\label{eq:jarlskog}
J = c_{12} c_{23} c_{13}^2 s_{12} s_{23} s_{13} \sin\delta \approx 3.18 \times 10^{-5}
\end{equation}
...
b_0 = 11 - \frac{2N_f}{3}
\end{equation}
with $N_f$ the number of active quark flavors. For $N_f \leq 16$, we have $b_0 > 0$ and the coupling decreases at high energy (asymptotic freedom).
\end{theorem}
...
\begin{enumerate}
    ...
    \item \textbf{Gluon loops} (non-Abelian gauge boson self-energy): contribute $+\frac{10}{3} \times 3 = +10$ to $b_0$ (for $\mathrm{SU}(3)$), tending to \emph{decrease} the coupling (anti-screening). This is the uniquely non-Abelian contribution.
    \item \textbf{Ghost loops} (Faddeev--Popov): contribute $+\frac{1}{3} \times 3 = +1$ to $b_0$.
\end{enumerate}
The total $b_0 = 11 - 2N_f/3$ is positive for $N_f < 17$; since the Standard Model has $N_f = 6$, $b_0 = 7 > 0$, and asymptotic freedom holds. This was discovered independently by Gross and Wilczek and by Politzer in 1973 (Nobel Prize 2004).
\end{proof}
...
Current global fits to the CKM matrix parameters give~\cite{PDG2024}:
\begin{align}
\lambda &= 0.22500 \pm 0.00067 \\
A &= 0.826^{+0.015}_{-0.015} \\
\bar{\rho} &= 0.159 \pm 0.010 \\
\bar{\eta} &= 0.348 \pm 0.010
\end{align}
