...
\begin{equation}
\label{eq:delta-rho}
\Delta\rho \approx \frac{3 G_F}{8\pi^2 \sqrt{2}} m_t^2 \approx 0.0095.
\end{equation}
\end{theorem}
...
\begin{equation}
m_p \approx 938~\text{MeV}, \qquad m_u + m_d + m_u \approx 9~\text{MeV}.
\end{equation}
...
\begin{equation}
\label{eq:rge}
\mu \frac{d\alpha_i^{-1}}{d\mu} = -\frac{b_i}{2\pi}, \qquad i = 1, 2, 3,
\end{equation}
with one-loop coefficients for the Standard Model ($n_g = 3$ generations, $n_H = 1$ Higgs doublet):
\begin{align}
...
\item $b_3 < 0$: $\su{3}_C$ coupling \emph{decreases} with energy (strongly asymptotically free). QCD confines at low energy.
\end{itemize}
...
\begin{enumerate}
\item The \textbf{flavor sector} --- the 22 free parameters of the Standard Model (6 quark masses, 3 charged lepton masses, 4 CKM parameters, 3 gauge couplings, the Higgs mass and VEV, the QCD $\theta$ parameter, and the cosmological constant) --- is not explained by any known compositional principle.
